...
#subscribers callbacks
    def laserCallback(self, lasermsg):
        ...
        for r in range(lengthLas):
            self.laserReading.append(sum(self.laserMsg.ranges[r*(self.noOfsamples/lengthLas):(r+1)*(self.noOfsamples/lengthLas)])/(self.noOfsamples/lengthLas))
    ...
    def publish(self, action):
        ...
        self.msgTwist.angular.z = action[0]
    ...
    def make_observaton(self):
        ...
        return Observation(focus=self.laserReading,
                           speedX=self.vel,
                           angle=self.ang)
